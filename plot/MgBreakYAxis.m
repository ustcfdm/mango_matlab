% Mango modified this code based on previous code. The initial code is from
...
%
function breakInfo = MgBreakYAxis(varargin)
    ...
    if strcmp(mainLayer,'bottom') %(!!!) is it only about layers?
        ...
        if numel(mainLegend) > 0
            lgnd = legend(highAxes,mainLegend.String);
%             set(lgnd, 'color', 'red')
        end
    ...
    theta = linspace(0,2*pi*4,16);
    xPoints = linspace(mainXLim(1),mainXLim(2),16);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    amp = splitHeight/2 * 0.9; % amp = splitHeight/2 * 0.9;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    patch(patchPointsX,patchPointsY ,figureColor,'Parent',breakAxes); %use of pathc(!!!)?
