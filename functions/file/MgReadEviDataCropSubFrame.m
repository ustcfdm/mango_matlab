function img = MgReadEviDataCropSubFrame(filename, fullWidth, fullHeight, idxFrameBegin, frameCount)
...
% idxFrameBegin: index start from 1.
...
img = zeros(fullHeight, fullWidth, frameCount);
rows = (1:info.Height) + info.ROI_Y;
cols = (1:info.Width) + info.ROI_X;

img(rows, cols, :) = MgReadRawFileSubRegion(filename, info.Height, info.Width, info.Nr_of_images, info.Offset_To_First_Image, info.Gap_between_iamges_in_bytes, dataType, ...
    [1, info.Height; 1, info.Width; idxFrameBegin, idxFrameBegin + frameCount - 1], false);
