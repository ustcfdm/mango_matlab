function [obj_folder_pmma_corr, obj_folder_no_corr] = MgBenchtopEviToSgmPmmaCorrBiasCorr_local_mean(config_filename, order_corr, binShape, badPixelCorr)
...
obj_folder_pmma_corr = sprintf('%s/PMMA_corr_bias_corr_%s_order_local_mean_%dx%d', js.EnergyBin, order_str{order_corr}, binShape(1), binShape(2));
obj_folder_no_corr = sprintf('%s/no_corr_bias_corr_%s_order_local_mean_%dx%d', js.EnergyBin, order_str{order_corr}, binShape(1), binShape(2));
...
for n = 1:numel(files_short)
    ...
    [prj, idx0] = MgLocalMeanReplace(prj, binShape);
    ...
    %-------------------------------
    for od = 1:order_corr
        prj_log(~idx0) = prj_log(~idx0) + corr_coeff(od) ./ (prj(~idx0).^od);
    end
