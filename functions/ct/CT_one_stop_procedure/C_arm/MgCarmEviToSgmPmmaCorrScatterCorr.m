function [obj_folder_pmma_corr, obj_folder_no_corr] = MgCarmEviToSgmPmmaCorrScatterCorr(config_filename, scatterPara)
% Generate sinogram from EVI data. Apply panel correction using pure PMMA.
% Apply scatter correction using simple Gaussian deconvolution

js = MgReadJsoncFile(config_filename);

%% folder names for sinogram (w and w/o PMMA correction)
obj_folder_pmma_corr = sprintf('%s/PMMA_corr_scatter_corr', js.EnergyBin);
obj_folder_no_corr = sprintf('%s/no_corr_scatter_corr', js.EnergyBin);
...
filename = sprintf('%s/cali_PMMA.mat', MgCarmGetPanelCorrProtocolFolder(js.PanelCorrProtocol));
...
prj_air = MgReadEviDataCrop(file_air, js.EviWidth, js.EviHeight);
...
for n = 1:numel(files_short)
    ...
    %===========================================================
    prj = MgReadEviDataCrop(files_long{n}, js.EviWidth, js.EviHeight);
    
    % apply scatter correction
    prj = MgScatterDeconvolution_test(prj, scatterPara);
    ...
    sgm_no_corr(:,1:760) = 0;
    sgm_no_corr(:,4100:end) = 0;
    
    sgm_pmma_corr(:,1:760) = 0;
    sgm_pmma_corr(:,4100:end) = 0;
    ...
    % save sinogram
    filename = strrep(files_short{n}, '.EVI', sprintf('_%d-%d-%d.raw', size(sgm_no_corr_rebin,2), size(sgm_no_corr_rebin,1), size(sgm_no_corr_rebin,3)));
